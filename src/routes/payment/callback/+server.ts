...
export async function POST({ request }) {
  ...
  const expectedHash = md5(
    `${TOYYIBPAY_SECRET_KEY}${status}${orderId}${refno}ok`,
  );
  ...
  if ((orderStatus === "paid" || orderStatus === "failed") && order?.user_id) {
    const posthog = getPostHogServer();
    if (posthog) {
      posthog.capture({
        distinctId: order.user_id,
        event: orderStatus === "paid" ? "payment_completed" : "payment_failed",
        properties: {
          order_id: orderId,
          bill_code: billcode,
          ref_no: refno,
          // ToyyibPay's callback amount is in cents like billAmount was
          // on creation — confirm against a real payload before trusting this.
          amount: amount ? Number(amount) / 100 : undefined,
          currency: "MYR",
          ...(orderStatus === "failed" ? { reason } : {}),
        },
      });
      // Cloudflare Workers can kill the isolate right after the Response
      // is returned — same class of bug as the CSP thing, just for
      // server-side capture instead of the browser. Await flush before
      // returning so the event actually leaves before the isolate dies.
      await posthog.flush();
    }
  }
