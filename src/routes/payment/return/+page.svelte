...
<script lang="ts">
  ...
  let pageTitle = $derived.by(() => {
    ...
    if (status === "failed")
      return "Pembayaran Tidak Berjaya - Akademi Abang Rumah";
...
<main class="bg-gray-50 flex items-center justify-center p-6 py-24">
  <div
    class="bg-white rounded-xl shadow-lg max-w-md w-full p-10 relative overflow-hidden"
  >
    <div
    ...
    {#if !data.order}
      <p
        class="text-xs font-mono uppercase tracking-widest text-[#4a7425] mb-3"
      >
        Tiada rekod
      </p>
      <h1 class="text-2xl font-extrabold text-gray-900 mb-4">
        Kami tidak jumpa pesanan itu
      </h1>
      <p class="text-sm text-gray-600 leading-relaxed">
        Jika akaun anda telah didebit, simpan resit ToyyibPay anda dan hubungi
        kami supaya kami boleh padankan pembayaran tersebut.
      </p>
    {:else if data.order.status === "paid"}
      ...
      <p class="text-sm text-gray-600 leading-relaxed mb-6">
        Terima kasih, {data.order.customer_name}. Pembayaran anda sebanyak RM {data.order.amount.toFixed(
          2,
        )} untuk {data.order.product_name} telah disahkan. Resit telah dihantar ke
        {data.order.customer_email}.
      </p>
      <p class="text-xs text-gray-400">
        Mengalihkan ke menu utama dalam {redirectSeconds}s…
        <a
          href={resolve("/")}
          class="text-[#4a7425] font-semibold hover:underline">pergi sekarang</a
        >
      </p>
    {:else if data.order.status === "failed"}
      <p class="text-xs font-mono uppercase tracking-widest text-red-600 mb-3">
        Tidak berjaya
      </p>
      <h1 class="text-2xl font-extrabold text-gray-900 mb-4">
      ...
      <p class="text-sm text-gray-600 leading-relaxed mb-6">
        Tiada apa-apa telah didebit. Anda boleh cuba lagi.
      </p>
      <p class="text-xs text-gray-400">
        Kembali ke checkout dalam {redirectSeconds}s…
        <a
    ...
    {:else}
      ...
      <h1 class="text-2xl font-extrabold text-gray-900 mb-4">
        Mengesahkan pembayaran anda&hellip;
      </h1>
      <p class="text-sm text-gray-600 leading-relaxed">
        Ini biasanya mengambil masa beberapa saat sahaja. Halaman ini akan
