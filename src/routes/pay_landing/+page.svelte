			...
			<form onsubmit={submit} class="bg-white rounded-xl shadow-lg p-8 flex flex-col gap-1">
				...
				{:else}
					...
					<p class="text-sm text-gray-600 mb-5 leading-relaxed">
						Log masuk dahulu untuk teruskan pendaftaran. Akaun ini akan digunakan untuk
						sahkan pembayaran dan beri akses terus ke video selepas bayaran berjaya.
					</p>
				{/if}
				...
				{:else}
					<a href="/login" class="bg-white text-gray-700 border border-gray-300 px-6 py-3.5 rounded-xl font-semibold
						       hover:bg-gray-50 transition flex items-center justify-center gap-3 w-full
						       disabled:opacity-60 disabled:cursor-not-allowed"> Log Masuk
					</a>

					<p class="text-xs text-gray-400 text-center mt-3.5 leading-relaxed">
						Selepas log masuk, anda akan kembali ke sini secara automatik untuk lengkapkan pembayaran.
					</p>
				{/if}
			</form>
		</div>
