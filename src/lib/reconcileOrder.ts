...
export async function reconcileOrder(orderId: toyyibpayID) {
	...
	const newStatus =
		tx.billpaymentStatus === '1' ? 'paid' : tx.billpaymentStatus === '3' ? 'failed' : 'pending';
	...
	if (updateError) {
		console.error('[reconcileOrder] order update failed:', updateError, { orderId });
	...
	if (newStatus === 'paid' && updatedOrder.user_id) {
		...
		if (profileError) {
			console.error('[reconcileOrder] profile update failed:', profileError, { orderId });
		}
	}
